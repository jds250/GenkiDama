...
func RlpHash(x interface{}) (h common.Hash) {
	...
	rlpRes, _ := rlp.EncodeToBytes(x)
	rlpHash := crypto.Keccak256Hash(rlpRes)
	return rlpHash
}
...
func (p *ProofElem) GetResHash() common.Hash {
	return p.Res.h
}

func (p *ProofElem) GetResDifficulty() *big.Int {
	return p.Res.td
}
...
func (p *ProofInfo) VerifyProof(blocks []*ProofBlock) bool {
	...
	jsonstr1, _ := json.Marshal(p.Elems)
	fmt.Println("json str proofs = ", string(jsonstr1))
	...
	for {
		if !proofs.is_empty() {
			...
			jsonstr, _ := json.Marshal(proof_elem)
			...
			if proof_elem.Cat == 2 {
				...
				if proof_elem.Right {
					fmt.Println("nodes len", len(nodes))
