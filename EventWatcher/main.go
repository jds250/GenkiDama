...
var OnChainLogPath = "./On-Eth-log-4.txt"
var OffChainLogPath = "./Off-Eth-log-4.txt"
...
func main() {
	...
	frAddress := common.HexToAddress(contract.EthFullRollupAddr)
	...
	for {
		...
		case vLog := <-logs:
			...
			case RollupBlockNotifyHash.Hex():
				...
				cur := time.Now()
				costlog.OnChainLog.Println("New Block-"+rbNotify.BlockNumber.String(), cur.UnixNano()/1e6, vLog.BlockNumber)
				break
			case LocalStateNotifyHash.Hex():
				...
				cur := time.Now()
				costlog.OnChainLog.Println("New LocalState", cur.UnixNano()/1e6, vLog.BlockNumber)
