...
contract LocalStateManager {
    ...
    function verifyAndLockLocalState(dt.LocalTransition[] memory _localTransitionList) public returns (bool) {

        dt.LocalTransition[] memory tmpTransitionList = new dt.LocalTransition[](
            _localTransitionList.length
        );
        uint256 tmpNum = 0;

        // decode all LocalState and verify
        for (uint256 i = 0; i < _localTransitionList.length; i++) {

            uint256 _chainID = _localTransitionList[i].chainID;
            uint256 _blockNum = _localTransitionList[i].blockNum;
            string memory _account = _localTransitionList[i].account;
            uint256 _value = _localTransitionList[i].value;
            uint256 _style = _localTransitionList[i].style;

            if (_chainID == chainID && _style == 1 && _value > localStatePool[_account].value) {
                emit ErrorNotify(1, "illegal transition: account balance is not enough");
                return false;
            }

            if (_chainID == chainID) {
                tmpTransitionList[tmpNum].chainID = _chainID;
                tmpTransitionList[tmpNum].blockNum = _blockNum;
                tmpTransitionList[tmpNum].account = _account;
                tmpTransitionList[tmpNum].value = _value;
                tmpTransitionList[tmpNum].style = _style;
    ...
    function verifyLocalState(dt.LocalTransition[] memory _localTransitionList) public returns (bool) {
