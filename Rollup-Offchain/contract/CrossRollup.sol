...
contract CrossRollup {
    ...
    modifier onlyArbiter() {
        require(
            msg.sender == address(arbiterAddress),
            "Only arbiter may perform action"
        );
        _;
    }
    ...
    onlySuper
    {
        require(_committerAddress != address(0), "committer address is zero");
        committerAddress = _committerAddress;
    }

    function setArbiterAddress(address _arbiterAddress) external onlySuper {
        require(_arbiterAddress != address(0), "arbiter address is zero");
        arbiterAddress = _arbiterAddress;
    }

    function isBlockInDispute(uint256 _blockNumber) public view returns (bool) {
        if (_blockNumber >= L2Blocks.length) {
            return false;
        }

        return block.number <= L2Blocks[_blockNumber].commitTime + DISPUTE_TIME;
    }

    function getBlockCommitmentHash(uint256 _blockNumber) public view returns (bytes32) {
        require(_blockNumber < L2Blocks.length, "block not exist");

        DataTypes.Block memory target = L2Blocks[_blockNumber];
        return keccak256(
            abi.encode(
                target.height,
                target.txRoot,
                target.localTransitionRoot,
                target.stateRoot,
                target.blockSize
            )
        );
    ...
    function commitBlock(
    ...
    ) external onlyCommitter returns (bool) {
        ...
        DataTypes.L2Transition[] memory expectedTransitions = myTE.GetL2Transitions(_transactions);
        tmpRoot = myDT.L2TransitionListHash(expectedTransitions);
        if (tmpRoot != _localTransitionRoot) {
            emit ErrorNotify(1, "Illegal transition list");
            return false;
        }
    ...
    function ExecutionChallenge(
        ...
        if (!isBlockInDispute(_targetBlockHeight)) {
            emit ErrorNotify(2, "challenge window closed");
            return;
        }
        ...
        if (!right) {
            ...
            _rollBackBlock(_targetBlockHeight);
            return;
        }
    ...
    // TODO: unrealize
    function _rollBackBlock(uint256 _targetBlockHeight) internal {
        if (_targetBlockHeight >= L2Blocks.length) {
            emit RollupBlockNotify(_targetBlockHeight, "3-1-rollback fail");
            return;
        }
        ...
        if (! success) {
            ...
            while (L2Blocks.length > _targetBlockHeight) {
                L2Blocks.pop();
            }

            if (confirmedNum > _targetBlockHeight) {
                confirmedNum = _targetBlockHeight;
            }
            emit RollupBlockNotify(_targetBlockHeight, "3-0-rollback success");
        }

    }

    function rollBackBlock(uint256 _targetBlockHeight) external onlyArbiter {
        _rollBackBlock(_targetBlockHeight);
    }
