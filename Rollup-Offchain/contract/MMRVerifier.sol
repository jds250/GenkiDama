...
contract MMRVerifier {
    ...
    function VerifyMMRProof(MMR_config memory _mmr, ProofInfo memory _info, ProofBlock[] memory _blocks) public pure returns (bool) {
...
//            tmpVerifyElem =nodes.pop();
    ...
    // TODO: verify the proof-block with proof info
    function VerifyMMRProof(ProofElement[] memory _elems, ProofBlock[] memory _blocks) public returns (bool) {
        // 排序，去重，同时倒置。默认由链下执行器在提交前完成
        ProofElement memory proofElem;
        ProofElement memory right_node;
        VerifyElem[] memory nodes = new VerifyElem[](2*_elems.length);
        bytes32 hash;

        // 设置索引
//        VerifyElem[] memory proofs = _info.Elems;
        uint front = 0;
        uint end = _elems.length;
        uint blocksEnd = _blocks.length;
        uint nodesEnd = 0;
        // blockIndex = _blocks.length - 1;

        // 如果元素不存在或者直接是根，那么默认错误
        ProofElement memory root_elem = _elems[end-1];
        end --;
        if (_elems.length == 0 || root_elem.Cat != 0) {
            return false;
        }
        ...
        if (_elems.length == 2) {
            if (_elems[0].Cat == 2) {
                return _elems[0].Res.h == _elems[1].Res.h;
            }
            return false;
        }
        ...
        while (end > front) {
            // 如果还有元素
            // 从proofs(_info.Elems)最前方取一个元素
//            ProofElement memory proof_elem = _elems[front];
            uint proof_elem_id = front;
            front ++;

            if (_elems[proof_elem_id].Cat == 2) { // 待证明元素是叶子节点
                ProofBlock memory proof_block = _blocks[blocksEnd-1];
                blocksEnd --;
                uint64 number = proof_block.Number;
                ...
                if (nodesEnd > 0) {
                    ...
                    uint256 left = proofElem.Res.td;
                    uint256 middle = root_elem.Res.td * proof_block.AggrWeight;
                    uint256 right = proofElem.Res.td + _elems[proof_elem_id].Res.td;
                ...
                if ((number%2 == 0) && (number != (root_elem.LeafNum-1))) {
                    ...
                    hash = merge2(_elems[proof_elem_id].Res.h, right_node.Res.h);
                    nodes[nodesEnd].Res.h = hash;
                    nodes[nodesEnd].Res.td = _elems[proof_elem_id].Res.td + right_node.Res.td;
                    nodes[nodesEnd].Index = number/2;
                    nodes[nodesEnd].LeafNumber = root_elem.LeafNum/2;
...
//                    left_node = nodes[nodesEnd];
                    ...
                    hash = merge2(nodes[nodesEnd].Res.h, _elems[proof_elem_id].Res.h);
                    nodes[nodesEnd].Res.h = hash;
                    nodes[nodesEnd].Res.td = _elems[proof_elem_id].Res.td + nodes[nodesEnd].Res.td;
                    nodes[nodesEnd].Index = number/2;
                    nodes[nodesEnd].LeafNumber = root_elem.LeafNum/2;
                    nodesEnd ++;
//                    nodes.push(tmpVerifyElem);
            ...
            } else if (_elems[proof_elem_id].Cat == 1) {
                if (_elems[proof_elem_id].Right) {
//                    left_node = nodes[nodesEnd];
                    nodesEnd --;
...
//                    nodes.push(tmpVerifyElem);
                    ...
                    nodes[nodesEnd].Res.h = _elems[proof_elem_id].Res.h;
                    nodes[nodesEnd].Res.td = _elems[proof_elem_id].Res.td;
                    nodes[nodesEnd].Index = 9999999;
                    nodes[nodesEnd].LeafNumber = 9999999;
                    nodesEnd ++;
                }
            } else if (_elems[proof_elem_id].Cat == 0){
                // do nothing
            } else {
                revert("invalid Cat...");
            }
...
//            tmpVerifyElem =nodes.pop();
    ...
    // getRoot 获取验证元素的根
    function getRoot(VerifyElem[] memory nodes) public pure returns(bytes32, uint256) {
    ...
    function merge2(bytes32 h1, bytes32 h2) public pure returns(bytes32) {
