...
contract MPTVerifier {
    ...
    function verifyProof(MerkleProof memory data) public pure returns (bool) {
        return _verifyProof(data, bytes(""), true);
    }

    function _verifyProof(
        MerkleProof memory data,
        bytes memory expectedNodeRef,
        bool verifyRoot
        ...
        } else if (!_matchesNodeReference(expectedNodeRef, node)) {
            return false;
        }
        ...
        if(numItems == 17) {
            ...
            if(data.keyIndex >= data.key.length) {
                ...
                bytes memory childRef = dec[index].toBytes();
                if(childRef.length != 0) {
                    data.keyIndex += 1;
                    data.proofIndex += 1;
                    return _verifyProof(data, childRef, false);
                }
            }
        } else if(numItems == 2) {
            ...
            if(prefix == 2) {
                ...
                if(keccak256(sharedNibbles) == keccak256(slice(actualKey, data.keyIndex, data.keyIndex + extensionLength))) {
                    ...
                    data.proofIndex += 1;
                    return _verifyProof(data, nodeValue, false);
                }
            } else if(prefix == 1) {
                // Odd Extension Node
    ...
    function _matchesNodeReference(bytes memory expectedNodeRef, bytes memory node) private pure returns (bool) {
        if (expectedNodeRef.length == 0) {
            return false;
        }

        if (expectedNodeRef.length < 32) {
            return keccak256(expectedNodeRef) == keccak256(node);
        }

        if (expectedNodeRef.length == 32) {
            return _toBytes32(expectedNodeRef) == keccak256(node);
        }

        return false;
    }

    function _toBytes32(bytes memory value) private pure returns (bytes32 result) {
        require(value.length == 32, "invalid bytes32");
        assembly {
